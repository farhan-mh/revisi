...
<?php
 include '../konek.php';
 $id = $_GET['id'];
$ambilData = mysqli_query($koneksi,"SELECT * FROM input8 WHERE id ='$id' ");
   while ($tampil = mysqli_fetch_array($ambilData)){
  ?>
  <h1 align="center">Edit Kolom</h1>
  <hr>
<p> <form action="" method="post" enctype="multipart/form-data">
    <b> Pilih File: </b><input type="file" name="NamaFile"  value="<?php echo $tampil['data'] ?>">
   <br> <br>
   ...
   <br><br><br>
   <a href="../halaman8.php" class="btn btn-warning">kembali</a>
...
<?php 
   ...
   if(isset($_POST['SIMPAN'])){
    ...
    move_uploaded_file($_FILES['NamaFile']['tmp_name'],$direktori.$file_name);
        ...
        mysqli_query($koneksi," UPDATE `input8` SET `data` = '$file_name', `tanggal_awal` = '$tgl_awal',
       `tanggal_akhir` = '$tgl_akhir', `jumlah` = '$jumlah', `nama` = '$nama' , `rencana` = '$rencana' 
        WHERE `input8`.`id` = '$id' ");

header("location:../halaman8.php?pesan=update");
   }else{
