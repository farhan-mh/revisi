...
<?php
 include '../konek.php';
 $id = $_GET['id'];
$ambilData = mysqli_query($koneksi,"SELECT * FROM input2 WHERE id ='$id' ");
   while ($tampil = mysqli_fetch_array($ambilData)){
  ?>
  <h1 align="center">Edit Kolom</h1>
  <hr>
<p> <form action=" " method="post" enctype="multipart/form-data">
    <b> Pilih File:<!-- </b><input type="file" name="data"> --><input type="file" name="NamaFile">
  <?php // $file_name = $_FILES['NamaFile']['name'];  ?>
   ...
   <br><br><br>
   <a href="../halaman2.php" class="btn btn-warning">kembali</a>
...
error_reporting(0);
   ...
   if(isset($_POST['SIMPAN'])){
    ...
    move_uploaded_file($_FILES['NamaFile']['tmp_name'],$direktori.$file_name);  
        ...
        mysqli_query($koneksi," UPDATE `input2` SET `data` = '$file_name', `tanggal_awal` = '$tgl_awal',
       `tanggal_akhir` = '$tgl_akhir', `jumlah` = '$jumlah', `nama` = '$nama' , `rencana` = '$rencana' 
        WHERE `input2`.`id` = '$id' ");

header("location:../halaman2.php?pesan=update");
   }
