...
<?php
 include '../konek.php';
 $id = $_GET['id'];
$ambilData = mysqli_query($koneksi,"SELECT * FROM input1 WHERE id ='$id' ");
...
<p> <form action="" method="post" enctype="multipart/form-data">
  ...
  <?php // $file_name = $_FILES['NamaFile']['name'];  ?>
   ...
   <br><br><br>
   <a href="../halaman1.php" class="btn btn-warning">kembali</a>
...
error_reporting(0);
   ...
   if(isset($_POST['SIMPAN'])){
    ...
    move_uploaded_file($_FILES['NamaFile']['tmp_name'],$direktori.$file_name);
        ...
        mysqli_query($koneksi," UPDATE `input1` SET `data` = '$file_name', `tanggal_awal` = '$tgl_awal',
       `tanggal_akhir` = '$tgl_akhir', `jumlah` = '$jumlah', `nama` = '$nama' , `rencana` = '$rencana' 
        WHERE `input1`.`id` = '$id' ");

header("location:../halaman1.php?pesan=update");
