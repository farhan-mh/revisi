...
<?php
 include '../konek.php';
 $id = $_GET['id'];
$ambilData = mysqli_query($koneksi,"SELECT * FROM input6 WHERE id ='$id' ");
...
error_reporting(0);
 ...
 if(isset($_POST['SIMPAN'])){
  ...
  move_uploaded_file($_FILES['NamaFile']['tmp_name'],$direktori.$file_name);
        ...
        mysqli_query($koneksi," UPDATE `input6` SET `data` = '$file_name', `tanggal_awal` = '$tgl_awal',
       `tanggal_akhir` = '$tgl_akhir', `jumlah` = '$jumlah', `nama` = '$nama' , `rencana` = '$rencana' 
        WHERE `input6`.`id` = '$id' ");

header("location:../halaman6.php?pesan=update");
 }
    ?>
